...
module ROB_svsim # ( 
    ...
    input   logic                               clk_i           ,       input   logic                               rst_i           ,       output  ROB_DP                              rob_dp_o        ,       input   DP_ROB                              dp_rob_i        ,       input   CDB     [C_CDB_NUM-1:0]             cdb_i           ,       output  ROB_AMT [C_RT_NUM-1:0]              rob_amt_o       ,       output  ROB_FL                              rob_fl_o        ,           input   logic                               exception_i     ,       input   logic   [C_THREAD_IDX_WIDTH-1:0]    thread_idx_i    ,       output  logic                               br_mis_valid_o  ,       output  logic   [C_XLEN-1:0]                br_target_o     ,       output  ROB_LSQ                             rob_lsq_o       ,   
  ...
  ROB ROB( {>>{ rob_mon_o }}, {>>{ clk_i }}, {>>{ rst_i }}, {>>{ rob_dp_o }}, 
        {>>{ dp_rob_i }}, {>>{ cdb_i }}, {>>{ rob_amt_o }}, {>>{ rob_fl_o }}, 
        {>>{ exception_i }}, {>>{ thread_idx_i }}, {>>{ br_mis_valid_o }}, 
        {>>{ br_target_o }}, {>>{ rob_lsq_o }}, {>>{ rob_head_mon_o }}, 
        {>>{ rob_tail_mon_o }}, {>>{ rt_pc_o }}, {>>{ rt_valid_o }}, 
        {>>{ rt_wfi_o }} );
endmodule
`endif
